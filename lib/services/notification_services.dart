...
class NotifyHelper {
  ...
  tz.TZDateTime _nextInstanceOfTenAM(int hour, int minutes, int remind, String repeat, String date) {
    ...
    tz.TZDateTime scheduledDate =
        tz.TZDateTime(tz.local, fd.year, fd.month, fd.day, fd.hour, fd.minute);
        
        remindAfter(remind, scheduledDate);
        
        
    if (scheduledDate.isBefore(now)) {
      if(repeat == "Daily"){
        scheduledDate = tz.TZDateTime(tz.local, now.year, now.month, (formattedDate.add(Duration(days: 1))).day, hour, minutes);
      }else if(repeat == "Weekly"){
        scheduledDate = tz.TZDateTime(tz.local, now.year, now.month, (formattedDate.add(Duration(days: 7))).day, hour, minutes);
      }else if(repeat == "Monthly"){
        scheduledDate = tz.TZDateTime(tz.local, now.year, (formattedDate.add(Duration(days: 30))).month, now.day, hour, minutes);
      }
      remindAfter(remind, scheduledDate);
  ...
  void remindAfter(int remind, tz.TZDateTime scheduledDate) {
    if(remind == 5){
